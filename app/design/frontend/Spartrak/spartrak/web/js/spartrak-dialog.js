 ...
 * ===========================================================================
 * `autoDismiss` (milliseconds) closes the dialog on its own, unconditionally —
 * nothing pauses it and nothing cancels it. The signed-out panel uses it; the
 * contact panel does not, and the difference is the point: one announces
 * something that already happened, the other is a reference the shopper opened
 * to read. See _startAutoDismiss for what that trade costs.
 *
 * ===========================================================================
 * WITHOUT THIS FILE
 * ===========================================================================
...
define([], function () {
    ...
    return function (config, element) {
        ...
        /**
         * ===================================================================
         * THE SELF-CLOSING TIMER
         * ===================================================================
         * `autoDismiss` (milliseconds) closes the dialog on its own, and it is
         * UNCONDITIONAL: once the panel is shown it goes away after that long,
         * full stop. Nothing pauses it and nothing cancels it.
         *
         * That is a deliberate product decision, and it is worth being explicit
         * about what it costs. An earlier build paused the countdown on pointer
         * movement and cancelled it on any key or press, to satisfy WCAG 2.2.1
         * (Timing Adjustable). Both guards are gone at the owner's instruction:
         * this panel is a confirmation of something that has ALREADY happened,
         * it asks for nothing, and the shopper loses nothing by it going away —
         * the page underneath is the store, which is where they were headed.
         *
         * A dialog that must be READ, or that asks a question, must not use
         * this. The contact panel passes no `autoDismiss` for exactly that
         * reason.
         *
         * Only the signed-out panel sets it today; a dialog that omits it is
         * never on a clock.
         */
        ...
        function released() {
            // A dialog dismissed by hand, by Escape or by the scrim must not
            // leave a timer running that would then fire against a closed
            // panel — and one that can be reopened (the contact panel is, from
            // three different links) must start clean each time.
            _clearAutoDismiss();
