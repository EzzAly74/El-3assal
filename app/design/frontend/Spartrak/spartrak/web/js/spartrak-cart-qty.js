...
define([
    ...
    $.widget('spartrak.cartQty', {
        ...
        _create: function () {
            this.element.find(this.options.line).each(function (index, line) {
        ...
        _enhanceLine: function ($line) {
            ...
            var $decrease = this._stepperButton('decrease', '−', $.mage.__('Decrease quantity'));
            var $value = $('<span/>', {
            ...
            var $increase = this._stepperButton('increase', '+', $.mage.__('Increase quantity'));

            $stepper.append($decrease, $value, $increase);
            $line.find(this.options.qtyField).append($stepper);
            $line.attr(this.options.stepperFlag, '');

            this._on($decrease, { click: function () { this._step($select, -1); }.bind(this) });
            this._on($increase, { click: function () { this._step($select, 1); }.bind(this) });
        ...
        _stepperButton: function (kind, glyph, label) {
            return $('<button/>', {
                type: 'button',
                'class': 'spartrak-cart-line__stepper-button spartrak-cart-line__stepper-button--' + kind,
                'aria-label': label,
                text: glyph
            });
        },
        ...
        _scheduleCommit: function () {
            ...
            this.commitTimer = window.setTimeout(function () {
                ...
                this._commit();
            }.bind(this), this.options.commitDelay);
        },
