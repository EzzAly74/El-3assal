...
class SetStatus extends Action implements HttpPostActionInterface
{
    ...
    public function execute(): Redirect
    {
        ...
        if ($entityId <= 0) {
            $this->messageManager->addErrorMessage(__('We could not find that branch.'));
        ...
        try {
            ...
            $this->messageManager->addSuccessMessage(
                $isActive ? __('The branch is now enabled.') : __('The branch is now disabled.')
            );
            ...
            $this->messageManager->addExceptionMessage(
                $exception,
                __('Something went wrong while updating the branch.')
            );
        }
